...
**MIDI como entrada principal, áudio como alternativa.** MIDI já é a informação exata que a transcrição de áudio tentaria adivinhar. Existe MIDI gratuito de quase tudo.
...
src/
  ...
  lib/
    midi.ts            parser (já escrito, sem dependências)
    bdo.ts             constantes verificadas do editor
    quantize.ts        MIDI → colunas do grid
    analyze.ts         orçamento de notas, alcance, compassos repetidos
    audio.ts           Basic Pitch, carregado sob demanda
  pages/
```

O parser de MIDI do protótipo é próprio e não tem dependência — migra direto para `lib/midi.ts`.
